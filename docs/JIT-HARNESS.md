...
This package expresses **Hermes Agent** as a typed, task-adaptive agent harness. Its concrete capability remains: Configure, extend, or contribute to Hermes Agent with explicit local/runtime safety boundaries. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Hermes Agent |
| --- | --- | --- |
| Memory | `layered_working_context` | Retain hermes configuration plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `layered_specification_plan` | Transform agent runtime into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_preview_authorize_execute_reread` | Advance the workflow through tools and models while preserving stop conditions and user authority. |
